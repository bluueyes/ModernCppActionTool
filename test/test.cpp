...
#include <iostream>
#include <numeric>
TEST(BaseTest,CalcTest){
    auto a = reaction::var(1);
    auto b = reaction::var(3.14);  
    EXPECT_EQ(a.get(), 1);
    EXPECT_EQ(b.get(), 3.14);

    auto c = reaction::calc([](int x, double y){return x+y;}, a, b);
    auto d = reaction::calc([](int x, double y){return std::to_string(x)+ std::to_string(y);}, a, c);

    ASSERT_FLOAT_EQ(c.get(), 4.14);
    EXPECT_EQ(d.get(), "14.140000");

    a.value(2);
    ASSERT_FLOAT_EQ(c.get(),5.14);
    EXPECT_EQ(d.get(),"25.140000");
...
TEST(ReactionTest1 ,TestAction){
    auto a = reaction::var(1);
    auto b = reaction::var(3.14);  
    EXPECT_EQ(a.get(), 1);
    EXPECT_EQ(b.get(), 3.14);

    auto at = reaction::action([](int x, double y){
